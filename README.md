...
**添加文件**：可以多选文件、选择整个文件夹（递归扫描其中的视频），或者把文件／文件夹直接拖进窗口。

**文件队列**：左侧列出所有待处理文件，逐个后台分析音轨。点某个文件即可查看它的音轨、时长、对齐信息。每条音轨前有单选钮，选定用于导出的那条；只有一条音轨的文件会自动选中。

**没选音轨时**：单音轨文件直接用它。多音轨文件且未选择时，点"开始输出"会先弹出提示，列出这些文件并说明将默认使用第一条音轨，确认后才执行。队列底部可关闭这一行为，关闭后必须先选音轨才能导出。

**输出位置**三档可选：

| 选项 | 实际位置 |
|---|---|
| 程序目录（默认） | `<程序目录>/TJ-Output/` |
| 各源文件所在目录 | 每个源文件旁边的 `TJ-Output/` 子文件夹 |
| 指定目录 | 你选择的目录 |
